...
## NB: this is slightly overkill as a simpler generator function would
## do but due to lack of time it was easier to copy-paste
class windowedGenotypeMatrixGenerator(tf.keras.utils.Sequence):
    """Generator class to provide windowed genotype matrices in batches"""
    def __init__(self, ts, maxlen, windows=None, batchSize=64, frameWidth=0):
        if windows is None:
            windows = [0.0, ts.sequence_length]
        self.windows = windows
        self.n_data = len(self.windows) - 1 # n-1 intervals
        self.maxlen = maxlen
        self.batchSize = batchSize
        self.frameWidth = frameWidth
        self.indices = np.arange(self.n_data)
        self.ts = ts
        self.variants = ts.variants()
        self._variant = None
    ...
    def __getitem__(self, index):
        """Generate data indices for a given batch index (where index <= len(self))"""
        batchIndices = self.indices[index * self.batchSize: (index+1) * self.batchSize]
        X = self.__data_generation(batchIndices)
        return X
    ...
    def __data_generation(self, batchIndices):
        ...
        pos = []
        for i in batchIndices:
            wstart = self.windows[i]
            wend = self.windows[i+1]
            H = []
            P = []
            while True:
                ...
                if v.position > wend:
                    self._variant = v
                    break
                if v.position < wstart:
                    self._variant = v
                    break
                H.append(v.genotypes)
                P.append(v.position)
            H = np.array(H, dtype="float32")
            P = np.array(P, dtype="float32")
            P = (P - wstart) / (wend - wstart)
            if H.shape[0] > self.maxlen:
                print(f"window interval {wstart}-{wend} has {H.shape[0]} segregating sites (max sites: {self.maxlen}); truncating!")
                H = H[0:self.maxlen, :]
                P = P[0:self.maxlen]
            hpad, ppad = pad(H, P, self.maxlen, frameWidth=self.frameWidth)
            haps.append(hpad)
