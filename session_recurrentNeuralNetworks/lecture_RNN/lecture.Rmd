...
```{r  tikz-rnn-recap-perceptron-multiout-simple, cache=FALSE, fig.ext="svg", out.width="800px", engine='tikz' }
\begin{tikzpicture}
  \tikzset{
...
```{r  tikz-rnn-recap-perceptron-multiout-simple-rotated, cache=FALSE, fig.ext="svg", out.height="600px", engine='tikz' }
\begin{tikzpicture}
  \tikzset{
    iolabel/.append style={rotate=0},
    nnconnection/.append style={->},
...
```{r  rnn-example-word-prediction, out.width="60%" }
...
```{r  rnn-example-language-translation, fig.ext="svg", out.width="100%", engine='tikz' }
...
```{r  airline-rnn-model-2, echo=FALSE, eval=TRUE, engine='python' }
model.summary() 
```
...
```{r  tikz-rnn-model-topology-writ-out-1, fig.ext='svg', out.width="80%", engine='tikz' }
...
```{r  airline-rnn-model-3, echo=FALSE, eval=TRUE, engine='python' }
model.summary() 
```

::::

```{r  tikz-rnn-model-topology-writ-out-2, fig.ext='svg', out.width="80%", engine='tikz' }
\begin{tikzpicture}[rotate=90, transform shape]
  \useasboundingbox (-0.5*\basenodesep, -12*\basenodesep) rectangle (6.5*\basenodesep, 11*\basenodesep);
  \tikzset{
    iolabel/.append style={rotate=-90},
...
```{r  tikz-karpathy-rnn-example-numbers, fig.ext='svg', out.width="50%", engine='tikz' }
\begin{tikzpicture}
  \tikzset{
...
```{r  tikz-big-gru, fig.ext="svg", fig.width=6, engine='tikz' }
\begin{tikzpicture}
\pic {gru};  
\end{tikzpicture} 
```
